...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  if (currentRole === 'customer') {
    ...
    return (
      <aside className="w-64 bg-white dark:bg-[#141619] border border-zinc-200 dark:border-zinc-800 rounded-3xl p-4 space-y-5 shrink-0 transition-colors shadow-sm flex flex-col justify-between min-h-[580px]">
        <div className="space-y-5">
          {/* Active Role Status Card */}
          <div className="p-3.5 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs space-y-1">
            <div className="flex items-center space-x-1.5 font-bold text-zinc-950 dark:text-zinc-50">
          ...
          <div>
            <div className="text-[11px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider px-3 mb-2 font-mono">
              Trang Khách Hàng
            </div>
            <nav className="space-y-1">
              {customerTabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-4 h-4 ${isActive ? 'text-white dark:text-zinc-950' : 'text-zinc-400'}`} />
                    <span>{tab.label}</span>
                  </button>
                );
              })}
        ...
        <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
          <button
            ...
            className="w-full py-2.5 px-3 rounded-xl bg-zinc-900 hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-100 text-white dark:text-zinc-950 text-xs font-bold transition-all shadow-sm flex items-center justify-center space-x-2"
          >
  ...
  const opTabs = visibleTabs.filter((t) =>
    ['dashboard', 'appointments', 'customers', 'cost_calculation', 'inventory'].includes(t.id)
  );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <Icon
                        className={`w-4 h-4 ${isActive ? 'text-white dark:text-zinc-950' : 'text-zinc-500 dark:text-zinc-400'}`}
                      />
                      <span>{tab.label}</span>
                    </div>
        ...
        {(currentRole === 'owner' || currentRole === 'manager') && onOpenEditSpaProfile && (
          <div className="pt-1">
            <button
              ...
              className="w-full flex items-center space-x-2.5 px-3 py-2.5 rounded-xl text-xs font-semibold text-amber-900 dark:text-amber-200 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 transition-all text-left"
